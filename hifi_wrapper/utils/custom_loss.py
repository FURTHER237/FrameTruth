...
class IsolatingLossFunction(torch.nn.Module):
	...
	def forward(self, model_output, label, threshold_new=None, update_flag=None):
		...
		self.dis_curBatch = pred_mask.to(model_output.device).to(torch.float32)
		...
		dist_mani = torch.max(torch.tensor(0).to(model_output.device).float(),
		...
		loss_nat  = dist_nat.sum()/label_nat_sum if label_nat_sum != 0 else \
					torch.tensor(0).to(model_output.device).float()
		loss_mani = dist_mani.sum()/label_mani_sum if label_mani_sum != 0 else \
					torch.tensor(0).to(model_output.device).float()
		loss_total = loss_nat + loss_mani

		return loss_total.to(model_output.device), loss_mani.to(model_output.device), loss_nat.to(model_output.device)
